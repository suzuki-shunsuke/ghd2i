...
complete -o bashdefault -o default -o nospace -F _cli_bash_autocomplete ghd2i`)
	return nil
}
...
func (cc *completionCommand) zshCompletionAction(*cli.Context) error {
	...
	fmt.Fprintln(cc.stdout, `#compdef ghd2i
...
if [ "$funcstack[1]" = "_ghd2i" ]; then
  _ghd2i "$@"
else
  compdef _ghd2i ghd2i
fi`)
	return nil
}
